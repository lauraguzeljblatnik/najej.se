...
def uvozi(recepti, sestavine, vrsta, priloznost, priprava):
    #odpremo CSV datoteko
    with open(recepti, 'r', encoding='utf-8') as p:
        with open(sestavine, 'r', encoding='utf-8') as f:
            with open(vrsta, 'r', encoding='utf-8') as v:
                with open(priloznost, 'r', encoding='utf-8') as f_pril:
                    with open(priprava, 'r', encoding='utf-8') as f_prip:
                        vrs_rec = csv.reader(p, delimiter = ',')
                        vrs_ses = csv.reader(f, delimiter = ',')
                        vrs_vrst = csv.reader(v, delimiter = ',')
                        vrs_pril = csv.reader(f_pril, delimiter = ',')
                        vrs_prip = csv.reader(f_prip, delimiter = ',')
                        ...
                        for r in vrs_rec: 
                            ...
                            [cas1] = (r[9].split('..'))
                            cas += [int(s) for s in cas1.split() if s.isdigit()]
                            postopek1 = (r[10])
                        ...
                        for r in vrs_vrst:
                            ...
                            vrst1= r[1].strip()
                            vrst.append(vrst1)
                            pom_vrst.append((int(id_recepta[0]), vrst1))
                        vrsta = set()
                        for el in vrst:
                            if el != '':
                                vrsta.add(el.strip())
                        ...
                        for r in vrs_pril:
                            ...
                            pril1= r[1].strip()
                            pril.append(pril1)
                            pom_pril.append((int(id_recepta[0]), pril1))
                        priloznost = set()
                        for el in pril:
                            if el != '':
                        ...
                        for r in vrs_prip:
                            ...
                            prip1= r[1].strip()
                            prip.append(prip1)
                            pom_prip.append((int(id_recepta[0]), prip1))
                        priprava = set()
                        for el in prip:
                            if el != '':
                        ...
                        for i in range(len(vrst)):
                            if vrst[i] !=  '':
                                cur.execute(
                                    """INSERT INTO vrsta_recepta(recept, vrsta)
                                        VALUES (%s, %s); """
                                    %(pom_vrst[i][0], uploaded_vrsta[vrst[i]])
                                )
                            ...
                            if pril[i] != '':
                                cur.execute(
                                    """INSERT INTO priloznost_recepta(recept, priloznost)
                                        VALUES (%s, %s); """
                                    %(pom_pril[i][0], uploaded_priloznost[pril[i]])
                                )
                            ...
                            if prip[i] !=  '':
                                cur.execute(
                                    """INSERT INTO priprava_recepta(recept, priprava)
                                        VALUES (%s, %s); """
                                    %(pom_prip[i][0], uploaded_priprava[prip[i]])
                                )
...
##
uvozi("recepti.csv", "sestavina.csv", "vrsta.csv", "priloznost.csv", "priprava.csv")
